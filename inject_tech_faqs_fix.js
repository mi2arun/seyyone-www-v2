...
techPages.forEach(page => {
  ...
  // Check if FAQ section is already physically injected into the JSX
  if (content.includes('Frequently Asked <span')) {
    console.log(`Skipping ${fullPath}, already has FAQ JSX.`);
    return;
  }
...
`;
  ...
  // Fix: some pages have {/* CTA */} instead of {/* CTA Section */}
  content = content.replace(/\{\/\*\s*CTA.*?\*\/\}/, match => faqSection + '\n        ' + match);

  fs.writeFileSync(fullPath, content);
  console.log(`Fixed JSX for ${fullPath}`);
});
